...
public interface WordControllerDocs {
    ...
    @Operation(summary = "비회원 단어 조회", description = "DB에 저장된 모든 단어를 조회합니다.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "단어 조회 성공"),
            @ApiResponse(responseCode = "204", description = "단어 조회 실패")})
    public Object getWordsByNonMember();
    ...
    @Operation(summary = "비회원 특정 단어 조회", description = "DB에 저장된 단어 중 특정 단어를 조회합니다..")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "단어 조회 성공"),
            @ApiResponse(responseCode = "404", description = "단어 조회 실패")})
    public Object getWordByNonMember(@RequestParam int wordId);
    ...
    public Object insertWord(@RequestHeader (value = "X-AUTH-TOKEN") String token, @RequestBody WordAddDTO wordInfo);
    ...
    public Object updateWord(@RequestHeader (value = "X-AUTH-TOKEN") String token, @RequestBody WordUpdateDTO wordInfo);
    ...
    @Operation(summary = "비회원 단어 검색", description = "검색어를 기준으로 단어를 조회합니다.")
    @ApiResponses(value = {
            @ApiResponse(responseCode= "200", description = "단어 검색 성공"),
            @ApiResponse(responseCode = "500", description = "단어 검색 실패")})
    public Object findWordByNonMember(@RequestParam String keyWord);
